...
- **The in-cluster registry has no authentication and no TLS.** It is reachable by anything that can route to the control-plane VIP. Acceptable only because that network is itself access-controlled; this does not belong on a network anyone untrusted can reach.
