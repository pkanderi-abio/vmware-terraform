...
⚠️ **Active issue**: a sweep of this cluster's worker nodes found ongoing ext4 journal corruption (hundreds to 1000+ abort events per node on 5 of 6 workers), affecting CSI-backed volumes across multiple unrelated workloads. The vSphere CSI driver itself has been ruled out — a full reinstall passed a complete lifecycle regression test (provision/attach/write/cross-node-reattach/delete) with zero errors. Current theory points to intermittent network-path instability between the cluster and vCenter (confirmed via repeated CSI-controller connection timeouts), possibly sharing a fabric with the storage array's iSCSI path — see `CLAUDE.md` for the full incident writeup. Not yet confirmed or resolved; treat CSI-backed workload data as at-risk until the underlying network path is investigated.
